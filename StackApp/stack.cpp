...
void InitializeStack(Stack *ps) { ps->StackTop = 0; }
...
int StackFull(Stack *ps) { return ps->StackTop == MAXSTACK; }

int StackEmpty(Stack *ps) { return ps->StackTop == 0; }

int StackSize(Stack *ps) { return ps->StackTop; }

void ClearStack(Stack *ps) { ps->StackTop = 0; }

void StackTop(StackEntry *pe, Stack *ps) { *pe = ps->entry[ps->StackTop - 1]; }
...
int StackEmpty(Stack *ps) { return ps->StackTop == NULL; }
...
void StackTop(StackEntry *pe, Stack *ps) { *pe = ps->StackTop->data; }
...
void Display(StackEntry e) { printf("%c", e); }
